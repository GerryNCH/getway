...
def init_db() -> None:
    ...
    with _conn() as conn:
        ...
        if "fun_facts_json" not in existing_cols:
            # 3-4 facts at the same bar as fun_fact — see
            # models.Itinerary.fun_facts. Same split: new routes get it
            # from the same Sonnet call; existing rows start empty and are
            # backfilled by main.py's startup task / POST /admin/backfill-fun-facts
            # (same endpoint that backfills the singular fun_fact).
            conn.execute("ALTER TABLE itineraries ADD COLUMN fun_facts_json TEXT DEFAULT '[]'")
...
def get_itinerary(video_id: str) -> Itinerary | None:
    ...
    return Itinerary(
        ...
        fun_facts=json.loads(row["fun_facts_json"] or "[]"),
        ...
        conn.execute(
            """INSERT OR REPLACE INTO itineraries
               (video_id, url, destination, duration, days_json, created_at,
                ...
                fun_fact, travel_tips_json, fun_facts_json)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                video_id,
                url,
                ...
                json.dumps(itinerary.fun_facts),
            ),
        )
...
def set_fun_facts(video_id: str, fun_facts: list[str]) -> bool:
    """
    Updates ONLY the fun_facts_json column for an existing route — used by
    the backfill path (main.py's startup task and POST
    /admin/backfill-fun-facts, the same endpoint that already backfills the
    singular fun_fact) to fill in the richer fact list for a route
    generated before this field existed. Returns False if video_id doesn't
    exist. Mirrors set_travel_tips exactly.
    """
    with _conn() as conn:
        cur = conn.execute(
            "UPDATE itineraries SET fun_facts_json = ? WHERE video_id = ?",
            (json.dumps(fun_facts), video_id),
        )
    return cur.rowcount > 0


def list_approved_missing_fun_facts() -> list[dict]:
    """
    Returns [{"video_id": ..., "destination": ...}, ...] for every approved
    route whose fun_facts is still empty. Mirrors
    list_approved_missing_travel_tips exactly.
    """
    with _conn() as conn:
        rows = conn.execute(
            """SELECT video_id, destination FROM itineraries
               WHERE status = 'approved' AND (fun_facts_json IS NULL OR fun_facts_json = '' OR fun_facts_json = '[]')"""
        ).fetchall()
    return [{"video_id": r["video_id"], "destination": r["destination"]} for r in rows]


def list_approved_for_fun_facts_refresh(force: bool = False) -> list[dict]:
    """
    Same as list_approved_missing_fun_facts(), except when force=True it
    returns EVERY approved route regardless of whether fun_facts is
    already filled. Mirrors list_approved_for_travel_tips_refresh exactly.
    """
    if not force:
        return list_approved_missing_fun_facts()
    with _conn() as conn:
        rows = conn.execute(
            """SELECT video_id, destination FROM itineraries WHERE status = 'approved'"""
        ).fetchall()
        ...
        cur = conn.execute(
            """UPDATE itineraries
               SET destination = ?, duration = ?, days_json = ?,
                   ...
                   car_rental_note = ?, fun_fact = ?, travel_tips_json = ?, fun_facts_json = ?
               WHERE video_id = ?""",
            (
                itinerary.destination,
                itinerary.duration,
