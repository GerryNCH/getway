...
class MonthDestination(BaseModel):
    name: str
    reason: str
    region: str = ""  # one of ai_analyzer._VALID_REGIONS — set from which
                       # per-region generate_month_calendar() call produced
                       # the entry, never parsed from the model's own
                       # output, so it's always populated for any row
                       # generated by the current code; "" only for a row
                       # cached under an older cache-key version (see
                       # database.py's versioned prefix), which a version
                       # bump already turns into a cache miss anyway
