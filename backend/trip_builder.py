...
def fits_budget(place: dict, budget: str) -> bool:
    ...
    if not price_level:
        # Neither ACTIVITY-TYPE results (guided tours, classes, boat trips —
        # see places.search_activities_by_type) NOR plain ATTRACTION results
        # reliably have Google's priceLevel set — that field is really meant
        # for restaurants/bars/hotels. This used to gate unpriced ACTIVITIES
        # on budget=="mid", which silently excluded nearly every genuine
        # activity whenever the traveler picked "cheap" or "luxury" — real
        # bug: this is why a Norway search once returned only 2 activities
        # total. The same gate was still applied to unpriced ATTRACTIONS,
        # though, and country-wide/less-touristy destinations (e.g. Norway
        # outside Oslo/Bergen) have plenty of genuine, well-known attractions
        # Google never priced AND that don't clear _is_famous's 2000-review
        # bar — real bug: this is why a Norway search once returned only 3
        # attractions total, for the identical reason already fixed for
        # activities above. Let both sections through regardless of tier
        # when there's no price signal; the AI curation pass's own
        # is_free/estimated_price judgment (real knowledge, not a blunt
        # Places-data guess) is a far better filter for these than assuming
        # "mid" by default.
        return True
...
3. For everything you keep, write ONE short, engaging sentence description (max ~15 words) in GetWay's tone — like a travel writer's pick, not a dry Google Maps category label. Never invent specific facts you can't reasonably infer from the name/type.
